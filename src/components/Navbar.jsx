  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled || !isHome ? "bg-[#011f4b]/95 backdrop-blur shadow-xl" : "bg-transparent"}`}>
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        {/* DESKTOP MENU */}
        <ul className="hidden md:flex gap-10 text-sm text-gray-300 font-medium">
          {navItems.map((item) => {
        ...
        {/* LOGIN BUTTON */}
        <button onClick={() => navigate("/login")} className="hidden md:block px-6 py-2 bg-white text-black rounded-full hover:bg-[#d29a23] hover:text-white transition-all duration-300 font-bold">
          Login
        </button>
