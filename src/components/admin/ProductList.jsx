...
const ProductList = () => {
    ...
    const filteredProducts = products.filter(p => {
        ...
        const matchesCategory = activeCategory === "All" || p.mainCategory === activeCategory;
        ...
        <div className="p-4 md:p-6 bg-[#00152b] min-h-screen text-white">
            <header className="flex flex-col space-y-6 border-b border-[#c7a17a]/20 pb-8 mb-8">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h2 className="text-3xl font-serif font-bold italic">Inventory Management</h2>
                        <p className="text-[#c7a17a] text-xs uppercase tracking-widest font-bold">Admin Control Panel</p>
                    </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                    {filteredProducts.map((product) => (
                        <div 
                            key={product._id} 
                            className="bg-[#001f3f] border border-[#c7a17a]/10 rounded-[2rem] overflow-hidden group relative"
                        >
                            ...
                            <div className="relative aspect-[4/5] bg-black/20 overflow-hidden">
                                ...
                                <div className="absolute bottom-4 left-6 flex flex-col gap-1">
                                    <span className="bg-[#c7a17a] text-[#001f3f] text-[7px] font-black px-2 py-0.5 rounded uppercase w-fit">
                                        {product.mainCategory}
                                    </span>
                                    <span className="bg-white/10 backdrop-blur-md text-white text-[8px] font-bold px-2 py-0.5 rounded uppercase w-fit">
                                        {product.subCategory}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="p-6 space-y-4">
                                <h3 className="font-serif font-bold text-xl truncate uppercase text-white">
                                    {product.name}
                                </h3>
                                <div className="flex justify-between items-center border-t border-white/5 pt-4">
                                    <p className="text-[#c7a17a] font-serif text-xl">₹{product.price.toLocaleString()}</p>
            ...
            {filteredProducts.length === 0 && !loading && (
                <div className="flex flex-col items-center justify-center py-32 text-[#c7a17a]/40">
                    ...
                    <p className="italic font-serif">Inventory is empty.</p>
                </div>
            )}
        </div>
    );
